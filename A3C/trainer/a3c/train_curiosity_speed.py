...
def train(rank, args, shared_model, shared_curiosity, shared_time_network, counter, lock, optimizer=None, optimizer_time=None, select_sample=True):
    ...
    if optimizer_time is None:
        optimizer_time = optim.Adam(list(shared_time_network.parameters()), lr=args.lr)

    curiosity = ICM(env.observation_space.shape[0], len(ACTIONS))
    time_network = ITM(env.observation_space.shape[0], len(ACTIONS))
    ...
    for num_iter in count():
        ...
        if rank == 0:
            ...
            if num_iter % args.save_interval == 0 and num_iter > 0:
                ...
                torch.save(shared_time_network.state_dict(), saveweights[:-4] + '_time_network.pkl')
